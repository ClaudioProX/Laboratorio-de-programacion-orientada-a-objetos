...
public class MotorCombate {
    ...
    public void ejecutarTurno(Personaje atacante, Personaje defensor) {
        ...
        if (atacante == null) {
            throw new PersonajeNuloException(
                    "ejecutarTurno(atacante)");
        }

        if (defensor == null) {
            throw new PersonajeNuloException(
                    "ejecutarTurno(defensor)");
        }
        ...
        try {
            ...
            bitacora.add("DERROTA | "
                    + e.getNombrePersonaje()
                    + " no pudo actuar");
            ...
            System.out.println(
                    "[ADVERTENCIA] Error de juego: "
                    + e.getMessage());

        } finally {
            System.out.println(
                    "[Bitacora] Turno registrado.");
        }
    }
